...
def bulk_delete_users():
    """
    Anonymise multiple users at once.
    Expects JSON: { "user_ids": [1, 2, 3] }
    """
    try:
        data = request.get_json()
        if not data or not isinstance(data.get('user_ids'), list):
            return jsonify({'msg': 'user_ids list required'}), 400

        ids = data['user_ids']
        if not ids:
            return jsonify({'msg': 'No user IDs provided'}), 400

        # Fetch existing, non‑admin, non‑anonymised users from the list
        ...
        return jsonify({
            'msg': f'Anonymised {anonymized_count} user(s)',
