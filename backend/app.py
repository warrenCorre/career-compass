...
def create_app():
    ...
    # On Railway the app root is writable; /tmp is always writable as fallback.
    ...
    try:
        ...
        # Fallback to /tmp if the instance folder is read-only (e.g. some Railway configs)
    ...
    # ── Load API mode from DB (safe – won't crash if DB isn't ready yet) ────
    with app.app_context():
        try:
            setting = Setting.query.filter_by(key='api_mode').first()
            app.config['API_MODE'] = setting.value if setting else 'MOCK_MODE'
            if not setting:
                db.session.add(Setting(key='api_mode', value='MOCK_MODE'))
                db.session.commit()
        except Exception as e:
            app.config['API_MODE'] = 'MOCK_MODE'
            logger.warning(f"Could not load API mode from DB (will use MOCK_MODE): {e}")
    ...
    with app.app_context():
        ...
        # Safely add last_activity column for existing deployments.
        # Uses text() for SQLAlchemy 2.x compatibility; swallows all errors
        # (column already exists, DB doesn't support ALTER TABLE, etc.)
        try:
            ...
            pass  # Column already exists or DB doesn't need it – that's fine
...
def seed_essential_data():
    ...
    try:
            ...
            for course_data in cat_courses:
                if not Course.query.filter_by(course_code=course_data['course_code']).first():
                    course = Course(
                        ...
                        description=(
                            f"Study {course_data['course_name']}. "
                            f"This program prepares students for careers in {cat_name.lower()}."
                        ),
        ...
        # ── Mock job listings (only seed if table is empty) ──────────────────
        if JobListing.query.count() == 0:
            ph_companies = {
                'tech':        ["Accenture Philippines", "IBM Philippines", "Globe Telecom", "PLDT", "GCash", "PayMaya", "Pointwest Technologies"],
                'healthcare':  ["St. Luke's Medical Center", "Makati Medical Center", "The Medical City", "Asian Hospital", "Philippine General Hospital"],
                'education':   ["Department of Education", "University of the Philippines", "Ateneo de Manila University", "De La Salle University"],
                'engineering': ["DMCI Homes", "EEI Corporation", "Meralco", "Aboitiz Power", "Ayala Land"],
                'business':    ["Jollibee Foods Corporation", "SM Investments", "BDO Unibank", "Metrobank", "BPI"],
            }
            locations = [
                "Makati City", "BGC, Taguig", "Ortigas, Pasig", "Quezon City",
                "Alabang, Muntinlupa", "Cebu City", "Davao City",
            ]
            job_titles = {
                'Technology':               ["Software Engineer", "Web Developer", "IT Support Specialist", "Data Analyst", "DevOps Engineer"],
                'Health & Medical Science': ["Staff Nurse", "Medical Technologist", "Pharmacist", "Physical Therapist"],
                'Education':                ["Elementary Teacher", "High School Teacher", "College Instructor", "Guidance Counselor"],
                'Engineering':              ["Civil Engineer", "Mechanical Engineer", "Electrical Engineer", "Project Engineer"],
                'Arts, Media, & Communication': ["Graphic Designer", "Content Writer", "Social Media Manager", "Video Editor"],
                'Social Sciences':          ["HR Associate", "Research Assistant", "Social Worker", "Policy Analyst"],
                'Hospitality & Tourism':    ["Hotel Manager", "Restaurant Manager", "Event Coordinator", "Tour Guide"],
                'Business & Management':    ["Accountant", "Financial Analyst", "Marketing Specialist", "Business Manager"],
            }
            skills_pool = {
                'tech':        ["Python", "JavaScript", "React", "Node.js", "SQL", "AWS", "Docker", "Git"],
                'healthcare':  ["Patient Care", "Vital Signs", "EMR", "CPR", "First Aid", "Medication Administration"],
                'education':   ["Lesson Planning", "Classroom Management", "Curriculum Development", "Student Assessment"],
                'engineering': ["AutoCAD", "Project Management", "Site Inspection", "Technical Drawing"],
                'business':    ["Financial Analysis", "Bookkeeping", "Tax Preparation", "MS Excel"],
            }
            industry_map = {
                'Technology': 'tech', 'Health & Medical Science': 'healthcare',
                'Education': 'education', 'Engineering': 'engineering',
                'Business & Management': 'business',
            }
            from datetime import timedelta
            for cat_name, titles in job_titles.items():
                industry = industry_map.get(cat_name, 'tech')
                companies = ph_companies.get(industry, ph_companies['tech'])
                pool      = skills_pool.get(industry, skills_pool['tech'])
                for i, title in enumerate(titles[:3]):
                    salary_min = random.choice([18000, 22000, 25000, 30000, 35000, 40000])
                    job = JobListing(
                        external_id=f"ph_mock_{cat_name.replace(' ', '_')}_{i}_{int(time.time()*1000)}_{random.randint(0,9999)}",
                        company=random.choice(companies),
                        title=title,
                        location=random.choice(locations),
                        description=f"We are looking for a {title} to join our team.",
                        skills=random.sample(pool, min(4, len(pool))),
                        salary_min=salary_min,
                        salary_max=salary_min + random.choice([5000, 8000, 10000, 15000]),
                        currency='₱',
                        job_url=f"https://www.jobstreet.com.ph/jobs/{i}",
                        source='ph_mock',
                        posted_at=datetime.utcnow() - timedelta(days=random.randint(1, 14)),
                    )
                    db.session.add(job)
            db.session.commit()
        ...
        if not existing_admin:
            admin = User(
                ...
                username=admin_username, email=admin_email, is_admin=True,
            )
