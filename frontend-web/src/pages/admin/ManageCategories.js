...
const ManageCategories = () => {
  ...
  const getCategoryImageUrl = (category) => {
    if (category.image_url) {
      if (category.image_url.startsWith('/')) {
        return `${BACKEND_URL}${category.image_url}`;
      }
  ...
  const handleEdit = (category) => {
    ...
    setFormData({
      ...
      image_file: null,
      image_preview: category.image_url ? (category.image_url.startsWith('/') ? `${BACKEND_URL}${category.image_url}` : category.image_url) : null,
  ...
  const handleMoveUp = async (index) => {
    ...
    try {
      await Promise.all([
        axios.put(`/api/admin/categories/${newCategories[index].id}`, { display_order: index }),
        axios.put(`/api/admin/categories/${newCategories[index - 1].id}`, { display_order: index - 1 }),
      ]);
          ...
          categories.map((cat, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute top-0 left-0 right-0 h-1 z-10" style={{ backgroundColor: colorOption.hex }} />
                ...
                <div className="relative p-6 z-10 flex flex-col min-h-[320px] text-white">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex space-x-1">
                      <button onClick={(e) => { e.stopPropagation(); handleMoveUp(idx); }} disabled={idx === 0}
                        className="p-1.5 bg-white/20 rounded-md text-white hover:bg-white/30 disabled:opacity-30 transition-all" title="Move Up">
                        <ArrowUpIcon className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); handleMoveDown(idx); }} disabled={idx === categories.length - 1}
                        className="p-1.5 bg-white/20 rounded-md text-white hover:bg-white/30 disabled:opacity-30 transition-all" title="Move Down">
                        <ArrowDownIcon className="h-3.5 w-3.5" />
                      </button>
                    </div>
                  </div>
                  
                  <h3 className="text-xl font-bold text-white mb-2 drop-shadow-lg">{cat.name}</h3>
                  <p className="text-sm text-white/80 mb-4 line-clamp-3 drop-shadow">{cat.description || 'No description provided'}</p>
                  
                  <div className="mt-auto flex items-center justify-end pt-4 border-t border-white/20">
                    <div className="flex space-x-2">
                      <button onClick={(e) => { e.stopPropagation(); handleViewDetails(cat); }}
                        className="p-2 bg-white/20 rounded-lg text-white hover:bg-white/30 transition-all" title="View Users">
                        <EyeIcon className="h-4 w-4" />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); handleEdit(cat); }}
                        className="p-2 bg-white/20 rounded-lg text-white hover:bg-white/30 transition-all" title="Edit">
                        <PencilIcon className="h-4 w-4" />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); handleDeleteClick(cat); }}
                        className="p-2 bg-white/20 rounded-lg text-white hover:bg-red-500/50 transition-all" title="Delete">
        ...
        {showModal && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => setShowModal(false)} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50" />
              ...
              <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto pointer-events-auto shadow-xl">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative w-20 h-20 rounded-lg overflow-hidden bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center">
                        {formData.image_preview ? (
                          <>
                            ...
                            <button onClick={removeImage} className="absolute top-0.5 right-0.5 p-0.5 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors">
                      ...
                      <div className="flex-1">
                        <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageSelect} className="hidden" id="category-image-upload" disabled={uploadingImage} />
                        <label htmlFor="category-image-upload" className={`inline-flex items-center px-3 py-1.5 bg-gray-100 text-gray-600 rounded-lg text-xs cursor-pointer hover:bg-gray-200 transition-colors ${uploadingImage ? 'opacity-50 cursor-not-allowed' : ''}`}>
                          <PhotoIcon className="h-3.5 w-3.5 mr-1" />
                          {uploadingImage ? 'Uploading...' : (formData.image_preview ? 'Change' : 'Upload')}
                        </label>
                        <p className="text-[10px] text-gray-400 mt-1">800x800px, max 5MB</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm" placeholder="e.g., Technology" />
                  </div>

                  {/* Description */}
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Description</label>
                    <textarea rows="2" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm" placeholder="Describe this category..." />
                  </div>
                ...
                <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 rounded-b-xl flex justify-end gap-2">
                  ...
                  <button onClick={handleSubmit} disabled={uploadingImage} className="px-4 py-2 bg-primary-500 text-white rounded-lg text-sm font-medium hover:bg-primary-600 transition-all disabled:opacity-50">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <div className="bg-white rounded-xl max-w-2xl w-full max-h-[85vh] overflow-y-auto pointer-events-auto shadow-xl">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="p-1.5 bg-primary-100 rounded-lg"><UsersIcon className="h-5 w-5 text-primary-600" /></div>
                        <h3 className="text-lg font-semibold text-gray-800">Users Interested in {selectedCategory.name}</h3>
                      </div>
                      <p className="text-sm text-gray-500 ml-11">
                        ...
                        {categoryUsers.length > 0 && (
                          <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-primary-100 text-primary-700">{categoryUsers.length} {categoryUsers.length === 1 ? 'user' : 'users'} found</span>
                        )}
                      </p>
                    </div>
                ...
                <div className="p-5 space-y-4">
                  {loadingUsers ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-400 text-sm max-w-md mx-auto">When students complete assessments in the <span className="font-medium">{selectedCategory.name}</span> category, they will appear here with their top program matches.</p>
                    ...
                    <>
                      <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-600 flex items-start gap-2">
                        ...
                        <span>Showing users who have completed assessments in this category. Match score indicates compatibility with the recommended program.</span>
                      </div>
                          ...
                          return (
                            <div key={idx} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-primary-50 transition-all group">
                              <div className="flex-shrink-0">
                                {profileImage ? (
                                  <img src={profileImage} alt={user.user_name} className="w-10 h-10 rounded-full object-cover ring-2 ring-white shadow-sm" />
                                ) : (
                                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center text-white font-semibold text-sm shadow-sm">{initials}</div>
                                )}
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                {user.top_match_course && <span className="text-xs text-gray-600 bg-white px-2 py-1 rounded-md border border-gray-200 group-hover:border-primary-200 transition-colors">{user.top_match_course}</span>}
                ...
                <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 rounded-b-xl flex justify-between items-center">
                  <p className="text-xs text-gray-400">{categoryUsers.length > 0 ? `Last updated: Just now` : ''}</p>
